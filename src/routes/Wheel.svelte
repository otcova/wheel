<script lang="ts">
    ...
    function beep(volume: number) {
        // Createing an 'Audio' element every time can lead to lag on mobile
        // So we store and reuse the 'Audio'.

        const sound = soundPool.pop() ?? new Audio(beepTrack);

        volume = Math.min(1, Math.max(0, volume));
        sound.volume = volume;
        sound.playbackRate = 0.9 + volume * 0.1;
        sound.preservesPitch = false;
        sound.play();
        sound.onended = () => soundPool.push(sound);
    }
